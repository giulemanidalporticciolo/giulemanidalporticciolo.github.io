...
let isProgrammaticCardScroll = false;
...
function updateMarkerDuringCardScroll() {
    if (isProgrammaticCardScroll) return;

    const currentScroll = infoTrack.scrollLeft;

    const viewportCenter =
        currentScroll +
        (infoTrack.clientWidth / 2);
...
function showPlace(marker, shouldScrollPage = true) {
    ...
    if (!infoWasVisible) {
        isProgrammaticCardScroll = true;

        infoTrack.scrollLeft =
            card.offsetLeft;

        requestAnimationFrame(() => {
            isProgrammaticCardScroll = false;
        });
